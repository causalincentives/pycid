...
class TestCPD(unittest.TestCase):
    ...
    def test_initialize_null_cpd(self):
        ...
        cpd_a.initializeTabularCPD(cid)
        self.assertTrue((cpd_a.get_values() == np.array([[0.5], [0.5]])).all())
        self.assertEquals(cpd_a.get_state_names('A', 1), 2)
        cpd_b = NullCPD('B', 2)
        cpd_b.initializeTabularCPD(cid)
    ...
    def test_initialize_function_cpd(self):
        ...
        self.assertEquals(cpd_a.get_cardinality(['A'])['A'], 1)
        self.assertEquals(cpd_a.get_state_names('A', 0), 2)

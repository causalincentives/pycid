...
from examples.simple_macids import basic2agent, basic2agent_tie_break, basic_different_dec_cardinality, \
    get_basic2agent_cyclic, get_basic_subgames, get_basic_subgames2, get_basic_subgames3, prisoners_dilemma, two_agent_no_pne, \
...
class TestMACID(unittest.TestCase):
    ...
    def test_pure_ne(self) -> None:
        ...
        macid2.add_cpds(*pne)
        #print(f"num of NE = {}")
        self.assertEqual(macid2.expected_utility({}, agent=1), 1)
        self.assertEqual(macid2.expected_utility({}, agent=2), 2)
        macid3 = two_agent_no_pne()
        self.assertEqual(len(macid3.get_all_pure_ne()), 0)

        macid_new = prisoners_dilemma()
        macid_new.get_all_pure_ne()
        print(macid_new.expected_utility({}, agent=1))
        print(macid_new.expected_utility({}, agent=2))
